...
@Controller('items')
@UseInterceptors(ClassSerializerInterceptor) //ハンドラーの実行前後にロジックを追加する。ClassSerializerInterceptorを利用することでハンドラーがレスポンスを返す前にuserEntityでexcludeをつけたパスワードを除外して、最終的なレスポンスを返す
export class ItemsController {
  ...
  async create(
    ...
    @GetUser() user: User, //JwtStrategyのvalidateメソッドでreturnされたリクエストオブジェクトのプロパティであるuserオブジェクト
  ): Promise<Item> {
